...
let endscreenContent = {win:'<span></span> you won!', loose: '💣 Boom! Game over.'};
...
const setup = () => {
  for(let i = 0; i < Math.pow(size, 2); i++ ) {
    ...
    board.appendChild(tile); //board의 자식에 tile 부여
  }
  ...
  tiles.forEach((tile, i) => {
    ...
    let random_boolean = Math.random() < bombFrequency;
      if(random_boolean) {
        bombs.push(`${x},${y}`);
        if(x > 0) numbers.push(`${x-1},${y}`); //x-position    
        if(x < boardSize -1) numbers.push(`${x+1},${y}`);
        if(y > 0) numbers.push(`${x},${y-1}`); //y-position
...
// titleSizeBtn.addEventListener('change', function(e) {
//   console.log(this.value);
//   tileSize = this.value;
//   clear();
// });
